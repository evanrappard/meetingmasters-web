...
import { POSTS_EN, getPostEN } from "../posts";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: url,
      // Zo weet Google dat dit hetzelfde artikel is in een andere taal, en
      // toont het de juiste versie aan de juiste lezer.
      languages: {
        "nl-NL": `${SITE}/nl/blog/${post.nlSlug}`,
        "en-GB": url,
      },
    },
    openGraph: {
...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const schema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE}/en/blog/${post.slug}` },
  };
      ...
      <section className="bg-[#F5F5F2] border-t border-[#E8E8E4] py-14 md:py-16">
        <div className="max-w-content mx-auto px-8 md:px-16 lg:px-20">
          <div className="flex items-end justify-between mb-8">
            <h2 className="font-bold text-[#2D2D2D] text-xl">More reading</h2>
            <Link href="/en/blog" className="text-sm font-bold text-[#2D2D2D] border-b-2 border-[#EEBE3D] pb-0.5 hover:border-[#2D2D2D] transition-colors">
              All articles →
            </Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {related.map((p) => (
              <Link key={p.slug} href={`/en/blog/${p.slug}`} className="group bg-white rounded-lg shadow-md overflow-hidden flex flex-col">
        ...
        <div className="max-w-content mx-auto px-8 md:px-16 lg:px-20">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-6 items-center">
            <div className="max-w-[560px]">
              <h2 className="font-bold text-white text-2xl leading-snug mb-2">Want a gathering people remember?</h2>
              <p className="text-white/60 text-base leading-relaxed">
                We design and run online events that genuinely matter. Tell us what you want to
                achieve.
              </p>
            </div>
            {/* Wijst voorlopig naar de Nederlandse adviespagina: die bestaat nog
                niet in het Engels. Omzetten zodra /en/expert-advies er is. */}
            <Link
