...
### Aanvulling: de uitleg stond op de verkeerde plek

Emilie zag de tekst niet. Terecht: haar schermafdruk was HubSpots eigen
boekingspagina, niet die van ons. Vanaf de adviespagina staat namelijk een link
**Kies een moment in de agenda** die HubSpot in een nieuw tabblad opent — en
daar hebben wij niets te zeggen, dus onze uitleg ging niet mee.

- **C** — De uitleg is losgetrokken naar `components/ui/AgendaUitleg.tsx` en
  staat nu op élke plek waar iemand de agenda tegenkomt: boven de ingesloten
  kalender op `/nl/demo` en `/en/demo`, én pal onder de agenda-link op
  `/nl/expert-advies` en `/en/expert-advice` — dus vóór de klik naar HubSpot.
