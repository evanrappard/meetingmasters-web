...
|---|---|---|---|
| Hero (achtergrondvideo) | `/videos/hero-boomerang-pauze.mp4` | LOKAAL | `Downloads/welcome hero stock.mp4` → 1080p, **20% vertraagd** (`setpts=1.25*PTS`), boomerang, **1,8 s rustpauze op beide keerpunten**, CRF 25, ~2,7 MB, 35,9 s. Opnieuw te maken met `node scripts/hero-video-pauze.mjs [seconden]`. Plain `<video autoplay muted loop playsinline>`, meteen instarten. **Uitsnede** `object-position: center 82%` (24 aug 2026, was 38% — beeld omhoog zodat de bubbels ruimer in beeld staan, keuze van Emilie uit een schuifproef); geldt ook voor de posters. Voorganger `hero-boomerang.mp4` (zonder pauze) staat er nog, wordt niet meer gebruikt |
